    ...
    return (
        ...
        min-h-[50vh] sm:h-fit py-12 px-6 flex gap-6'>
            ...
            <div className="rightSide text-white text-[1vmin] w-fit justify-end xl:w-1/2 flex flex-wrap gap-2">
                ...
                {
                    weatherData?.slice(1, 5).map((weather:any, index:number) => {
                        return (
...
{/* 
                <div className="city1 bg-gradient-to-r from-cyan-500/50 to-blue-500/50 flex flex-col gap-10 rounded-lg w-[48%] h-1/2 py-8 px-6">
                    <div className="row1 text-[12px] flex justify-between">
                        <div className="humiditySide">
                            <p>Precepitation: 2%</p>
                            <p>Humidity: {weatherData[1]?.main.humidity}%</p>
                            <p>Wind Speed: {weatherData[1]?.wind.speed}</p>
                        </div>
                        <div className="location">
                            <p>{weatherData[1]?.name}, {weatherData[1]?.sys.country}</p>
                            <p>{today}</p>
                            <p>{timeNow}</p>
                        </div>
                    </div>
                    <div className="row2 text-center flex justify-center items-center gap-10">
                        <div className="icon text-[25px]">
                            <Image className='w-24 h-24' src={`https://openweathermap.org/img/wn/${weatherData[1]?.weather[0].icon}@4x.png`} alt='icon' width={300} height={300} />
                        </div>
                        <h1 className='text-[25px]'>{weatherData[1]?.main.temp}<sup className=''>o</sup>C</h1>
                    </div>
                </div>

                <div className="city2 bg-gradient-to-r from-green-300/50 to-green-700/50 flex flex-col gap-10 rounded-lg w-[48%] h-1/2 py-8 px-6">
                    <div className="row1 text-[12px] flex justify-between">
                        <div className="humiditySide">
                            <p>Precepitation: 2%</p>
                            <p>Humidity: {weatherData[2]?.main.humidity}%</p>
                            <p>Wind Speed: {weatherData[2]?.wind.speed}</p>
                        </div>
                        <div className="location">
                            <p>{weatherData[2]?.name}, {weatherData[2]?.sys.country}</p>
                            <p>{today}</p>
                            <p>{timeNow}</p>
                        </div>
                    </div>
                    <div className="row2 text-center flex items-center justify-center gap-10">
                        <div className="icon text-[25px]">
                            <Image className='w-24 h-24' src={`https://openweathermap.org/img/wn/${weatherData[2]?.weather[0].icon}@4x.png`} alt='icon' width={300} height={300} />

                        </div>
                        <h1 className='text-[25px]'>{weatherData[2]?.main.temp}<sup className=''>o</sup>C</h1>
                    </div>
                </div>

                <div className="city3 bg-gradient-to-r from-violet-500/50 to-fuchsia-500/50 flex flex-col gap-10 rounded-lg w-[48%] h-1/2 py-8 px-6">
                    <div className="row1 text-[12px] flex justify-between">
                        <div className="humiditySide">
                            <p>Precepitation: 2%</p>
                            <p>Humidity: {weatherData[3]?.main.humidity}%</p>
                            <p>Wind Speed: {weatherData[3]?.wind.speed}</p>
                        </div>
                        <div className="location">
                            <p>{weatherData[3]?.name}, {weatherData[3]?.sys.country}</p>
                            <p>{today}</p>
                            <p>{timeNow}</p>
                        </div>
                    </div>
                    <div className="row2 text-center flex justify-center items-center gap-10">
                        <div className="icon text-[25px]">
                            <Image className='w-24 h-24' src={`https://openweathermap.org/img/wn/${weatherData[3]?.weather[0].icon}@4x.png`} alt='icon' width={300} height={300} />

                        </div>
                        <h1 className='text-[25px]'>{weatherData[3]?.main.temp}<sup className=''>o</sup>C</h1>
                    </div>
                </div>

                <div className="city4 bg-gradient-to-r from-red-400/50 to-red-700/50 flex flex-col gap-10 rounded-lg  w-[48%] h-1/2 py-8 px-6">
                    <div className="row1 text-[12px] flex justify-between">
                        <div className="humiditySide">
                            <p>Precepitation: 2%</p>
                            <p>Humidity: {weatherData[4]?.main.humidity}%</p>
                            <p>Wind Speed: {weatherData[4]?.wind.speed}</p>
                        </div>
                        <div className="location">
                            <p>{weatherData[4]?.name}, {weatherData[4]?.sys.country}</p>
                            <p>{today}</p>
                            <p>{timeNow}</p>
                        </div>
                    </div>
                    <div className="row2 text-center flex justify-center items-center gap-10">
                        <div className="icon text-[25px]">
                            <Image className='w-24 h-24' src={`https://openweathermap.org/img/wn/${weatherData[4]?.weather[0].icon}@4x.png`} alt='icon' width={300} height={300} />

                        </div>
                        <h1 className='text-[25px]'>{weatherData[4]?.main.temp}<sup className=''>o</sup>C</h1>
                    </div>
                </div> */}
            </div>
        </div>
